...
def list_counters():
    ...
    counters = [dict(name=count[0], counter=count[1]) for count in COUNTER.items()]  # noqa: E501
...
def create_counters(name):
    ...
    if name in COUNTER:
        return abort(status.HTTP_409_CONFLICT, f"Counter {name} already exists")  # noqa: E501

    COUNTER[name] = 0

    location_url = url_for("read_counters", name=name, _external=True)  # noqa: E501
    return (
...
def read_counters(name):
    ...
    if name not in COUNTER:
        return abort(status.HTTP_404_NOT_FOUND, f"Counter {name} does not exist")  # noqa: E501
